...
namespace Ok99\PrivateZoneCore\AdminBundle\Date;
...
class MomentFormatConverter extends \Sonata\CoreBundle\Date\MomentFormatConverter
{
    ...
    public function convert($format)
    {
        if (!array_key_exists($format, $this->phpMomentMapping)) {
            throw new InvalidParameterException(sprintf("PHP Date format '%s' is not a convertible moment.js format; please add it to the 'Ok99\PrivateZoneCore\AdminBundle\Date\MomentFormatConverter' class by submitting a pull request if you want it supported.", $format));
        }
